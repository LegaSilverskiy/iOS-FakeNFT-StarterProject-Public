...
final class ProfileViewController: UIViewController, ProfileViewProtocol {
    ...
    private func routeToMyNft() -> UIViewController {
        ...
        let favService = FavouritesService(networkClient: networkClient)
        let presenter = MyNftPresenter(nftService: nftService, favService: favService, profile: profile)
    ...
    private func routeToFavourites() -> UIViewController {
        guard let profile = presenter.profile else { return UIViewController()}
        ...
        let myNftViewController = FavouritesViewController(presenter: presenter)
        presenter.delegate = self
        presenter.view = myNftViewController
        return myNftViewController
    }
...
extension ProfileViewController: UITableViewDelegate {
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if indexPath.row == 0 {
            ...
            let favouritesViewController = routeToFavourites()
...
extension ProfileViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let favsCount = presenter.profile?.likes.count {
            if indexPath.row == 1 {
                cell.title.text = "\(presenter.tableNames[indexPath.row]) (\(favsCount))"
            }
        }
...
// MARK: - SendInfoDelegate

extension ProfileViewController: SendInfoDelegate {
