...
private extension CatalogViewController {
    ...
    func showAlertController() {
        ...
        let sortByName = UIAlertAction(title: .sortingOptionsByNamedNft, style: .default) { _ in
             self.presenter.sortByName()
         }
         let sortByNftCount = UIAlertAction(title: .sortingOptionsByNumberOfNFTs, style: .default) { _ in
...
extension CatalogViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        guard let cell = tableView.dequeueReusableCell(
            withIdentifier: CustomCellForTableView.reUseIdentifier,
            for: indexPath
        ) as? CustomCellForTableView else {
            return UITableViewCell()
        }
        let params = presenter.getParamsForCell(
            for: indexPath.row
        )
        cell.configure(
            with: params
